...
void HideWindow::connectTitleBar(){
    connect(tb, &QCustomTitleBar::closeRequest, this, [this](){
        emit closed();
        this->close();
    });
    connect(tb, &HideTitleBar::hideBtnClicked, this, [this](){ emit hided(); });
...
bool HideWindow::eventFilter(QObject *, QEvent *event){
    switch (event->type()) {
        case QEvent::Leave:
...
void HideWindow::customMouseMoveEvent(QMouseEvent *event){
...
void HideWindow::setTitleBar(HideTitleBar* _tb)
{
    tb = _tb;
    tb->setParent(this);
...
void HideWindow::redefineCursor(const QPoint &pos){
...
void HideWindow::paintEvent(QPaintEvent *event){
    QStyleOption opt;
    opt.initFrom(this);
    QPainter p(this);
